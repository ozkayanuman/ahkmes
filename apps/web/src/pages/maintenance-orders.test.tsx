...
vi.mock("../lib/api", async () => {
  ...
  return { ApiError: actual.ApiError, apiGet: (...args: unknown[]) => apiGet(...args), apiPost: (...args: unknown[]) => apiPost(...args) };
});
...
const asset = { id: "machine-1", assetCode: "CNC-001", name: "Torna 1", model: "QT-200", manufacturer: "Mazak", serialNumber: "MZ-42", criticality: "HIGH", maintenanceState: "BREAKDOWN", productionAllowed: false, plant: { id: "plant-1", name: "Ana Fabrika" }, workCenter: { id: "wc-1", name: "Talaşlı İmalat" }, reliability: { machines: [{ failureCount: 3, mttrHours: 2.5, mtbfCalendarHours: 48 }] } };
...
const order = { id: "mo-1", bakNo: "BAK-0001", machine: asset, type: "CORRECTIVE", priority: "CRITICAL", status: "IN_PROGRESS", description: "Spindle arızasını gider", actualStart: "2026-08-17T09:20:00.000Z", technicianAssignments: [{ id: "a-1", isPrimary: true, technician: { id: "tech-1", name: "Teknisyen A" } }] };
...
describe("MaintenanceOrdersPage — CNC-V1-07R workbench", () => {
  beforeEach(() => {
    ...
    apiGet.mockReset().mockImplementation((path: string) => {
      ...
      if (path === "/maintenance-orders/mo-1") return Promise.resolve(currentDetail);
      if (path === "/maintenance-orders/assets/machine-1") return Promise.resolve({ ...asset, activeBreakdown: breakdown, openMaintenanceOrders: [order], history: { requests: [request], breakdowns: [breakdown], orders: [order], downtime: [downtime] } });
  ...
  it("shows required KPIs, queues and filters without /machines", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Takvim PM üret" })).toBeInTheDocument();
    expect(await screen.findByRole("button", { name: "Runtime PM kontrolü" })).toBeInTheDocument();
  ...
  it("uses canonical request, breakdown conversion and WO detail actions", async () => {
    ...
    expect(screen.getByLabelText("Yedek parça raf kimliği")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Rezervasyonu bırak" }));
    expect(apiPost).toHaveBeenCalledWith("/maintenance-orders/mo-1/spares/spare-1/reservations/reservation-1/cancel", {});
    await user.click(screen.getByRole("button", { name: "Görevi tamamla: Koruyucuyu kilitle" }));
    expect(apiPost).toHaveBeenCalledWith("/maintenance-orders/mo-1/tasks/task-1/complete", {});
  });

  it("uses explicit plan command for a draft work order", async () => {
    currentDetail = { ...detail, status: "DRAFT" };
    const user = userEvent.setup(); renderPage(); await screen.findByText("CMMS Bakım Workbench");
    await user.click(screen.getByRole("button", { name: /Bakım iş emirleri/ }));
    await user.click(screen.getByRole("button", { name: "BAK-0001 ayrıntısını aç" }));
    await user.click(await screen.findByRole("button", { name: "Planla" }));
    expect(apiPost).toHaveBeenCalledWith("/maintenance-orders/mo-1/plan", expect.objectContaining({ idempotencyKey: expect.any(String) }));
  });
