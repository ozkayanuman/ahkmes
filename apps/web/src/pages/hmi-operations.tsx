...
type MaterialStatus = "NONE" | "READY" | "SHORTAGE";
type Operation = {
...
const materialStatusLabel: Record<MaterialStatus, string> = { NONE: "Malzeme yok", READY: "Malzeme hazır", SHORTAGE: "Malzeme eksik" };
const materialStatusBadge: Record<MaterialStatus, string> = { NONE: "bg-slate-100 text-slate-500", READY: "bg-green-100 text-green-800", SHORTAGE: "bg-red-100 text-red-800" };
type Detail = Operation & {
  ...
  executionHistory: Array<{ id: string; type: string; createdAt: string; reasonCode?: string | null; note?: string | null; actor: { id: string; name: string } }>;
  reworkRequirements: Array<{ id: string; quantity: string; processedQty: string; status: string; nonConformance: { id: string } }>;
  materialRequirements: Array<{
    id: string; itemType: "MATERIAL" | "PART"; itemId: string; unit: string; status: string; issueMethod: string;
    requiredQty: string; reservedQty: string; issuedQty: string; consumedQty: string; returnedQty: string; scrappedQty: string;
    item?: { id: string; code?: string; partNo?: string; name: string; unit: string; lotTrackingRequired?: boolean } | null;
    reservations: Array<{ id: string; status: string; quantity: string; issuedQty: string; bin: { code: string; warehouse?: { name: string } | null }; lot?: { lotNo: string } | null }>;
  }>;
  controllerVerification: { required: boolean; verification: "MATCH" | "MISMATCH" | "UNVERIFIED" | "STALE" | "UNSUPPORTED"; reason?: string; expectedProgramIdentity?: string; observedProgramIdentity?: string | null; observation?: { ingestedAt: string; connectionState: string; machineState: string; trustLevel: string; alarmCode?: string | null; alarmText?: string | null; capabilities: Record<string, boolean> } };
...
Object.assign(statusLabel, { SETUP: "Setup", PAUSED: "Paused", HELD: "Held", REWORK: "Rework" });
Object.assign(badge, { SETUP: "bg-violet-100 text-violet-800", PAUSED: "bg-amber-100 text-amber-800", HELD: "bg-red-100 text-red-800", REWORK: "bg-orange-100 text-orange-800" });
...
function OperationDetail({ detail, loading, error, canStart, canComplete, startPending, completePending, onChanged, onStart, onComplete }: { detail?: Detail; loading: boolean; error: boolean; canStart: boolean; canComplete: boolean; startPending: boolean; completePending: boolean; onChanged: () => void; onStart: () => void; onComplete: (payload: { goodCount: number; scrapCount: number; notes?: string }) => void }) {
  ...
  const materialShortage = detail.materialStatus === "SHORTAGE";
  const startDisabled = !canStart || detail.status !== "PENDING" || blockers.length > 0 || !!detail.activeRun || materialShortage;
  ...
  return <div className="space-y-4"><Card><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-semibold">{detail.workOrder.woNo} · {detail.seq}. {detail.name}</h2><p className="mt-1 text-sm text-slate-600">{detail.workOrder.part.partNo} rev. {detail.workOrder.part.revision} · {detail.workOrder.part.name} · plan {fmtQty(detail.workOrder.quantity)}</p></div><span className={`rounded px-3 py-1 text-sm font-semibold ${badge[detail.status]}`}>{statusLabel[detail.status]}</span></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div><span className="text-slate-500">Makine</span><div className="font-medium">{detail.machine?.name ?? "Atanmadı"}</div></div><div><span className="text-slate-500">Çalışma birimi</span><div className="font-medium">{detail.workCenter?.name ?? "Tanımlı değil"}</div></div><div><span className="text-slate-500">Termin</span><div className="font-medium">{fmtDate(detail.workOrder.dueDate)}</div></div></div></Card>
    {detail.controllerVerification.required && <Card className={detail.controllerVerification.verification === "MATCH" ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}><h3 className="font-semibold">CNC controller verification</h3><div className="mt-2 grid gap-2 text-sm sm:grid-cols-2"><span>Expected NC: <b>{detail.controllerVerification.expectedProgramIdentity ?? "—"}</b></span><span>Observed NC: <b>{detail.controllerVerification.observedProgramIdentity ?? "—"}</b></span><span>Result: <b>{detail.controllerVerification.verification}</b></span><span>Connection: <b>{detail.controllerVerification.observation?.connectionState ?? "UNKNOWN"}</b></span><span>Observed: {detail.controllerVerification.observation ? fmtDate(detail.controllerVerification.observation.ingestedAt) : "—"}</span><span>Machine: {detail.controllerVerification.observation?.machineState ?? "UNKNOWN"}</span></div>{detail.controllerVerification.reason && <p className="mt-2 text-sm font-medium">{detail.controllerVerification.reason}</p>}{detail.controllerVerification.observation?.alarmText && <p className="mt-2 text-sm text-red-800">Alarm: {detail.controllerVerification.observation.alarmText}</p>}<p className="mt-2 text-xs text-slate-600">Read-only controller evidence never creates MES quantities or starts a physical CNC cycle.</p></Card>}
    ...
    {detail.instructionHtml && <Card><h3 className="font-semibold">İş Talimatı</h3><div className="mt-3"><InstructionEditor content={detail.instructionHtml} onChange={() => {}} readOnly /></div></Card>}
    <MaterialRequirements requirements={detail.materialRequirements} />
    ...
    {detail.status === "PENDING" && <Card><h3 className="font-semibold">Operasyonu başlat</h3>{blockers.length > 0 && <p className="mt-2 flex gap-2 text-sm text-red-700"><AlertTriangle className="h-4 w-4 shrink-0" />{blockers.join(" ")}</p>}{materialShortage && <p className="mt-2 flex gap-2 text-sm text-red-700"><AlertTriangle className="h-4 w-4 shrink-0" />Gerekli üretim malzemesi tam tahsis edilmedi; aşağıdaki "Üretim malzemeleri" bölümünden rezerve/ver işlemi tamamlanmalı.</p>}{!canStart && <p className="mt-2 text-sm text-red-700">HMI_START action grant'i yok.</p>}<Button className="mt-4 h-12 px-7" disabled={startDisabled || startPending} onClick={onStart}><Play className="h-5 w-5" /> Başlat</Button></Card>}
    ...
    <LifecycleControls detail={detail} onChanged={onChanged} />
    <Card><h3 className="font-semibold">Execution history</h3>{detail.executionHistory.length === 0 ? <p className="mt-2 text-sm text-slate-500">No lifecycle events yet.</p> : <ol className="mt-3 space-y-2">{detail.executionHistory.map((event) => <li key={event.id} className="rounded border border-slate-200 p-2 text-sm"><b>{event.type}</b> · {fmtDate(event.createdAt)} · {event.actor.name}{event.reasonCode ? ` · ${event.reasonCode}` : ""}{event.note ? ` · ${event.note}` : ""}</li>)}</ol>}</Card>
    {detail.machine && <DowntimeCard machineId={detail.machine.id} canStart={canStart} />}
  </div>;
}

function MaterialRequirements({ requirements }: { requirements: Detail["materialRequirements"] }) {
  const readyCount = requirements.filter((requirement) => Number(requirement.reservedQty) + Number(requirement.issuedQty) >= Number(requirement.requiredQty)).length;
  return <Card><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-semibold">Üretim malzemeleri</h3>{requirements.length > 0 && <span className={`rounded px-2 py-1 text-xs font-semibold ${readyCount === requirements.length ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>{readyCount} / {requirements.length} kalem hazır</span>}</div><p className="mt-1 text-sm text-slate-500">Tahsis ve fiziksel çıkış durumu salt okunurdur. Lot-takipli kalemlerde lot bilgisi tahsis kaydından gelir.</p>{requirements.length === 0 ? <p className="mt-3 text-sm text-slate-500">Bu iş emri için malzeme gereksinimi yok.</p> : <div className="mt-3 space-y-3">{requirements.map((requirement) => {
    const itemLabel = requirement.item ? `${requirement.item.code ?? requirement.item.partNo ?? requirement.item.id} · ${requirement.item.name}` : requirement.itemId;
    const required = Number(requirement.requiredQty);
    const allocated = Number(requirement.reservedQty) + Number(requirement.issuedQty);
    const allocationMissing = allocated < required;
    const issuedPct = required > 0 ? Math.min(100, (Number(requirement.issuedQty) / required) * 100) : 0;
    const reservedPct = required > 0 ? Math.min(100 - issuedPct, (Number(requirement.reservedQty) / required) * 100) : 0;
    const missingLot = requirement.item?.lotTrackingRequired && requirement.reservations.some((reservation) => !reservation.lot);
    return <div key={requirement.id} className={`rounded border p-3 text-sm ${allocationMissing ? "border-amber-200 bg-amber-50" : "border-slate-200"}`}><div className="flex flex-wrap items-start justify-between gap-2"><div><b>{itemLabel}</b><div className="mt-1 text-xs text-slate-600">{requirement.itemType === "MATERIAL" ? "Malzeme" : "Alt montaj"} · {requirement.issueMethod} · durum: {requirement.status}</div></div><span className={`rounded px-2 py-1 text-xs font-semibold ${allocationMissing ? "bg-amber-100 text-amber-900" : "bg-green-100 text-green-800"}`}>{allocationMissing ? "Tahsis eksik" : "Tahsis tamam"}</span></div><div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-slate-200"><div className="flex h-full"><div className="h-full bg-green-500" style={{ width: `${issuedPct}%` }} /><div className="h-full bg-blue-300" style={{ width: `${reservedPct}%` }} /></div></div><div className="mt-3 grid gap-2 text-xs sm:grid-cols-4"><span>Gerekli <b>{fmtQty(requirement.requiredQty)} {requirement.unit}</b></span><span>Tahsis <b>{fmtQty(requirement.reservedQty)} {requirement.unit}</b></span><span>Çıkış <b>{fmtQty(requirement.issuedQty)} {requirement.unit}</b></span><span>Tüketim <b>{fmtQty(requirement.consumedQty)} {requirement.unit}</b></span></div>{missingLot && <p className="mt-2 text-xs font-medium text-red-700">Lot-takipli kalemin tahsisinde lot bilgisi eksik; issue yapılmamalı.</p>}{requirement.reservations.length > 0 && <div className="mt-3 space-y-1 border-t pt-2 text-xs text-slate-600">{requirement.reservations.map((reservation) => <div key={reservation.id}>Raf {reservation.bin.warehouse?.name ? `${reservation.bin.warehouse.name} / ` : ""}{reservation.bin.code} · lot {reservation.lot?.lotNo ?? "—"} · {fmtQty(reservation.quantity)} {requirement.unit} ({reservation.status})</div>)}</div>}</div>;
  })}</div>}</Card>;
}

function LifecycleControls({ detail, onChanged }: { detail: Detail; onChanged: () => void }) {
  const toast = useToast();
  ...
  const key = (action: string) => `${action}-${detail.id}-${crypto.randomUUID()}`;
  const command = useMutation({ mutationFn: ({ path, body }: { path: string; body: Record<string, unknown> }) => apiPost(`/hmi/operations/${detail.id}${path}`, body), onSuccess: () => { toast("Lifecycle action recorded.", "success"); onChanged(); }, onError: (error) => toast(errorMessage(error, "Lifecycle action failed."), "error") });
  const submit = (path: string, body: Record<string, unknown>) => command.mutate({ path, body });
  const lifecycleBody = (action: string) => ({ idempotencyKey: key(action), ...(reasonCode ? { reasonCode } : {}), ...(note ? { note } : {}) });
  const firstOpenRework = detail.reworkRequirements.find((item) => item.status === "OPEN");
  return <Card><h3 className="font-semibold">Controlled execution</h3><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><Label>Reason</Label><Input value={reasonCode} onChange={(event) => setReasonCode(event.target.value)} /></div><div><Label>Note</Label><Input value={note} onChange={(event) => setNote(event.target.value)} /></div></div><div className="mt-4 flex flex-wrap gap-2">
    {detail.status === "PENDING" && <><Button disabled={command.isPending} onClick={() => submit("/setup/start", lifecycleBody("setup-start"))}>Start setup</Button><Button disabled={command.isPending} onClick={() => submit("/start", {})}>Start</Button></>}
    {detail.status === "SETUP" && <Button disabled={command.isPending} onClick={() => submit("/setup/complete", lifecycleBody("setup-complete"))}>Complete setup</Button>}
    {detail.status === "IN_PROGRESS" && <><Button variant="outline" disabled={command.isPending} onClick={() => submit("/pause", lifecycleBody("pause"))}>Pause</Button><Button variant="outline" disabled={command.isPending} onClick={() => submit("/hold", lifecycleBody("hold"))}>Hold</Button></>}
    {detail.status === "PAUSED" && <><Button disabled={command.isPending} onClick={() => submit("/resume", lifecycleBody("resume"))}>Resume</Button><Button variant="outline" disabled={command.isPending} onClick={() => submit("/hold", lifecycleBody("hold"))}>Hold</Button></>}
    {detail.status === "HELD" && <Button disabled={command.isPending} onClick={() => submit("/hold/release", lifecycleBody("hold-release"))}>Release hold</Button>}
    {detail.status === "REWORK" && firstOpenRework && <Button disabled={command.isPending} onClick={() => submit("/rework/report", { ...lifecycleBody("rework-report"), quantity: Number(reworkQty) })}>Report rework</Button>}
    {detail.status !== "REWORK" && firstOpenRework && <Button disabled={command.isPending} onClick={() => submit("/rework/start", { ...lifecycleBody("rework-start"), reworkRequirementId: firstOpenRework.id })}>Start rework</Button>}
  </div>{detail.status === "IN_PROGRESS" && <div className="mt-4 grid gap-3 sm:grid-cols-3"><div><Label>Good delta</Label><Input type="number" min="0" value={goodQty} onChange={(event) => setGoodQty(event.target.value)} /></div><div><Label>Scrap delta</Label><Input type="number" min="0" value={scrapQty} onChange={(event) => setScrapQty(event.target.value)} /></div><div className="flex items-end"><Button disabled={command.isPending} onClick={() => submit("/reports", { ...lifecycleBody("production-report"), goodQty: Number(goodQty), scrapQty: Number(scrapQty) })}>Report production</Button></div></div>}{detail.status === "REWORK" && <div className="mt-4"><Label>Rework quantity</Label><Input type="number" min="0" value={reworkQty} onChange={(event) => setReworkQty(event.target.value)} /></div>}{firstOpenRework && <p className="mt-3 text-sm text-amber-800">Rework requirement: {firstOpenRework.processedQty} / {firstOpenRework.quantity}; NCR {firstOpenRework.nonConformance.id}</p>}</Card>;
}
