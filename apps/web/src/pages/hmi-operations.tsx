...
function MaterialRequirements({ requirements }: { requirements: Detail["materialRequirements"] }) {
  return <Card><h3 className="font-semibold">Üretim malzemeleri</h3><p className="mt-1 text-sm text-slate-500">Tahsis ve fiziksel çıkış durumu salt okunurdur. Lot-takipli kalemlerde lot bilgisi tahsis kaydından gelir.</p>{requirements.length === 0 ? <p className="mt-3 text-sm text-slate-500">Bu iş emri için malzeme gereksinimi yok.</p> : <div className="mt-3 space-y-3">{requirements.map((requirement) => {
