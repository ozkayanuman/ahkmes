...
export function MaintenanceOrdersPage() {
  ...
  const command = useMutation({
    mutationFn: ({ path, body }: { path: string; body?: Entity }) => apiPost(path, body ?? {}),
  ...
  const normalizeAssignments = (item: Entity): Entity => ({ ...item, assignments: (item.assignments ?? item.technicianAssignments ?? []).map((assignment: Entity) => ({ ...assignment, user: assignment.user ?? assignment.technician })) });
  const rows = { requests: wb.requests ?? [], breakdowns: wb.breakdowns ?? [], orders: (wb.orders ?? []).map(normalizeAssignments), pm: due.data ?? wb.pmDue ?? [], downtime: downtime.data ?? wb.activeDowntime ?? [] };
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {can(ACTION.pm) && <Button variant="outline" onClick={() => command.mutate({ path: "/maintenance-orders/plans/generate", body: { asOf: new Date().toISOString() } })}><RotateCcw className="h-4 w-4" />Takvim PM üret</Button>}
        {can(ACTION.pm) && <Button variant="outline" onClick={() => command.mutate({ path: "/maintenance-orders/predictive-check", body: {} })}><RotateCcw className="h-4 w-4" />Runtime PM kontrolü</Button>}
      </div>
    </header>
    ...
    <Modal open={requestOpen} title="Bakım talebi bildir" onClose={() => setRequestOpen(false)}><EntityForm assets={assets.data ?? []} form={form} setForm={setForm} problem priorityLabel="Talep önceliği" submitLabel="Talebi kaydet" onSubmit={() => submit("/maintenance-orders/requests", { machineId: form.machineId, problem: form.problem, priority: form.priority, ...(form.description ? { description: form.description } : {}), idempotencyKey: key() }, () => setRequestOpen(false))} /></Modal>
    <Modal open={breakdownOpen} title="Arıza bildir" onClose={() => setBreakdownOpen(false)}><EntityForm assets={assets.data ?? []} form={form} setForm={setForm} problem priorityLabel="Arıza önceliği" submitLabel="Arızayı kaydet" onSubmit={() => submit("/maintenance-orders/breakdowns", { machineId: form.machineId, description: form.problem, priority: form.priority, failureStartedAt: form.failureStartedAt || new Date().toISOString(), productionImpact: "PRODUCTION_STOP", idempotencyKey: key() }, () => setBreakdownOpen(false))} breakdown /></Modal>
    <Modal open={!!selectedOrderId} title={`Bakım iş emri ${orderDetail.data?.bakNo ?? ""}`} onClose={() => setSelectedOrderId(undefined)} className="max-h-[90vh] max-w-4xl overflow-y-auto">{orderDetail.data && <OrderDetail data={normalizeAssignments(orderDetail.data)} can={can} run={(p, b) => command.mutate({ path: p, body: b })} />}</Modal>
    <Modal open={!!selectedAssetId} title={`Varlık ${assetDetail.data?.assetCode ?? assetDetail.data?.name ?? ""}`} onClose={() => { setSelectedAssetId(undefined); setRtsOpen(false); }} className="max-h-[90vh] max-w-4xl overflow-y-auto">{assetDetail.data && <AssetDetail data={assetDetail.data} history={assetDetail.data.history} canRts={can(ACTION.rts)} rtsOpen={rtsOpen} setRtsOpen={setRtsOpen} form={form} setForm={setForm} submit={() => submit(`/maintenance-orders/assets/${selectedAssetId}/return-to-service`, { reason: form.rtsNotes, notes: form.rtsNotes, idempotencyKey: key() }, () => setRtsOpen(false))} />}</Modal>
  </div>;
}
...
function OrderDetail({ data, can, run }: { data: Entity; can: (a: string) => boolean; run: (path: string, body?: Entity) => void }) {
  ...
  return <div className="space-y-6"><div className="grid gap-3 sm:grid-cols-3"><Card className="p-4"><b>Durum</b><div>{data.status}</div></Card><Card className="p-4"><b>Makine</b><div>{data.machine?.name}</div></Card><Card className="p-4"><b>Teknisyen</b><div>{data.assignments?.map((a: Entity) => a.user?.name).join(", ") || "Atanmadı"}</div></Card></div>
    ...
    <section><h3 className="mb-2 font-semibold">Yedek parçalar</h3>{can(ACTION.spare) && <div className="mb-2 max-w-sm"><Input aria-label="Yedek parça raf kimliği" placeholder="Raf kimliği" value={local.binId ?? ""} onChange={(e) => setLocal({ ...local, binId: e.target.value })} /></div>}{(data.spareLines ?? []).map((s: Entity) => <div key={s.id} className="flex flex-wrap items-center justify-between gap-2 border-b py-2 text-sm"><span>{s.itemName ?? s.item?.name}</span><span>Plan {s.plannedQuantity} · Rezerve {s.reservedQty ?? "0"} · Çıkış {s.issuedQty ?? s.issuedQuantity ?? "0"} · İade {s.returnedQty ?? s.returnedQuantity ?? "0"}</span>{can(ACTION.spare) && <span className="flex flex-wrap gap-1"><Button size="sm" variant="outline" disabled={!local.binId} onClick={() => run(`/maintenance-orders/${data.id}/spares/${s.id}/reservations`, { quantity: "1", binId: local.binId, idempotencyKey: key() })}>Rezerve et</Button>{(s.reservations ?? []).filter((reservation: Entity) => reservation.status === "OPEN").map((reservation: Entity) => <Button key={reservation.id} size="sm" variant="outline" onClick={() => run(`/maintenance-orders/${data.id}/spares/${s.id}/reservations/${reservation.id}/cancel`)}>Rezervasyonu bırak</Button>)}<Button size="sm" variant="outline" disabled={!local.binId} onClick={() => run(`/maintenance-orders/${data.id}/spares/${s.id}/issue`, { quantity: "1", binId: local.binId, idempotencyKey: key() })}>Çıkış</Button><Button size="sm" variant="outline" disabled={!local.binId} onClick={() => run(`/maintenance-orders/${data.id}/spares/${s.id}/return`, { quantity: "1", binId: local.binId, idempotencyKey: key() })}>İade</Button></span>}</div>)}</section>
    {can(ACTION.assign) && <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); run(`/maintenance-orders/${data.id}/assignments`, { technicianId: local.technicianId, isPrimary: true }); }}><Input aria-label="Teknisyen kullanıcı kimliği" placeholder="Teknisyen kullanıcı kimliği" value={local.technicianId ?? ""} onChange={(e) => setLocal({ ...local, technicianId: e.target.value })} /><Button type="submit">Teknisyen ata</Button></form>}
    {can(ACTION.execute) && <div className="flex flex-wrap gap-2">{data.status === "DRAFT" && <Button onClick={() => run(`/maintenance-orders/${data.id}/plan`, { idempotencyKey: key() })}>Planla</Button>}{data.status === "PLANNED" && <Button onClick={() => run(`/maintenance-orders/${data.id}/release`, { idempotencyKey: key() })}>Serbest bırak</Button>}{data.status === "RELEASED" && <Button onClick={() => run(`/maintenance-orders/${data.id}/start`, { idempotencyKey: key() })}>Başlat</Button>}{data.status === "ON_HOLD" && <Button onClick={() => run(`/maintenance-orders/${data.id}/resume`, { idempotencyKey: key() })}>Devam et</Button>}{data.status === "IN_PROGRESS" && <Button variant="outline" onClick={() => run(`/maintenance-orders/${data.id}/hold`, { idempotencyKey: key() })}>İş emrini beklet</Button>}</div>}
...
function AssetDetail({ data, history, canRts, rtsOpen, setRtsOpen, form, setForm, submit }: { data: Entity; history?: Entity; canRts: boolean; rtsOpen: boolean; setRtsOpen: (v: boolean) => void; form: Entity; setForm: (v: Entity) => void; submit: () => void }) { const reliability = data.reliability?.machines?.[0]; return <div className="space-y-5"><div className={`rounded-lg border p-4 ${data.productionAllowed === false ? "border-red-300 bg-red-50" : "border-green-300 bg-green-50"}`}><div className="font-semibold">Üretim izni: {data.productionAllowed === false ? "BLOKE" : "İZİNLİ"}</div><div className="text-sm">Bakım durumu: {data.maintenanceState ?? "AVAILABLE"}</div>{data.productionAllowed === false && <p className="mt-2 text-sm font-medium text-red-700">Controller READY durumu bakım blokajını kaldırmaz.</p>}</div>{canRts && data.productionAllowed === false && <Button onClick={() => setRtsOpen(true)}><ShieldAlert className="h-4 w-4" />Üretime geri al</Button>}{rtsOpen && <form className="space-y-2 rounded-lg border p-4" onSubmit={(e) => { e.preventDefault(); submit(); }}><Label htmlFor="rts-notes">Servise dönüş notu</Label><Textarea id="rts-notes" required value={form.rtsNotes ?? ""} onChange={(e) => setForm({ ...form, rtsNotes: e.target.value })} /><Button type="submit">Servise dönüşü onayla</Button></form>}{reliability && <section><h3 className="mb-2 font-semibold">Güvenilirlik</h3><div className="grid gap-3 sm:grid-cols-3"><Card className="p-4"><b>Arıza</b><div className="mt-1 text-2xl">{reliability.failureCount}</div></Card><Card className="p-4"><b>MTTR</b><div className="mt-1 text-2xl">{reliability.mttrHours == null ? "—" : `${Number(reliability.mttrHours).toFixed(1)} sa`}</div></Card><Card className="p-4"><b>Arızalar arası</b><div className="mt-1 text-2xl">{reliability.mtbfCalendarHours == null ? "—" : `${Number(reliability.mtbfCalendarHours).toFixed(1)} sa`}</div><div className="text-xs text-slate-500">Takvim aralığıdır</div></Card></div></section>}<section><h3 className="mb-2 flex items-center gap-2 font-semibold"><History className="h-4 w-4" />Bakım geçmişi</h3><div className="grid gap-3 sm:grid-cols-2">{[["Talepler", history?.requests], ["Arızalar", history?.breakdowns], ["İş emirleri", history?.orders], ["Duruşlar", history?.downtime]].map(([title, items]) => <Card key={String(title)} className="p-4"><b>{title}</b><div className="mt-1 text-2xl">{Array.isArray(items) ? items.length : 0}</div></Card>)}</div></section></div>; }
