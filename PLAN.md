...
- [~] **Faz K — Çok seviyeli BOM (alt montaj) desteği, tam genealogy zinciri (2026-08-07 başladı):** Önceki "MTU/seri numarası bazlı takip yok (veri modelinde yok)" kısıtının kök nedeni: `BomLine`/`MaterialConsumption` sadece `materialId` destekliyordu (Part-of-Part alt montaj yok), bu yüzden `lots.service.ts`/`serial-numbers.service.ts` `trace()` zinciri tek hop'ta doğal olarak sonlanıyordu. 3 fazlı plan onaylandı:
  ...
  - **Faz 2b — Part tüketimi (YAPILMADI, kapsam Faz 2 planlanandan geniş çıktı):** `MaterialConsumption`'ı `itemType`/`itemId` desenine taşımak ilk düşünüldüğünden daha geniş bir alanı etkiliyor: sadece `consumption.service.ts` değil, `work-orders.service.ts` (`genealogy()`/`cost()` — ikisi de `.material` include'u kullanıyor), `lots.service.ts` (`trace()`, backward dalı) ve 5 frontend dosyası (`lots.tsx`, `serial-numbers.tsx`, `genealogy.tsx`, `work-order-detail.tsx`, `genealogy.test.tsx`) de `MaterialConsumption.material`'a dayanıyor. `InventoryService.record()` zaten polimorfik (`itemType: PART` destekliyor, `PartStock`'u doğru düşürüyor) — asıl iş `MaterialConsumption` şemasını taşımak + bu 5+ tüketici noktasını uyarlamak. Faz 3 (trace recursive hale getirme) ile birlikte ele alınması daha mantıklı — ayrı bir kapsam kararı gerekiyor.
  - **Faz 3 (YAPILMADI):** `lots.service.ts`/`serial-numbers.service.ts` `trace()` uygulama-katmanı recursive hale getirilir, frontend ağaç görünümü, çok seviyeli e2e senaryosu.
